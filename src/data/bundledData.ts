// This file is auto-generated. Do not edit directly.
import type { IngredientDatabase, System, Setting } from '../types';

export const defaultDatabase: IngredientDatabase = {
  ingredients: {
    acetyl_alcohol: {
      name: 'Acetyl Alcohol',
      id: 'acetyl_alcohol',
      categories: ['emollient_alcohols'],
    },
    amino_methyl_propanol: {
      name: 'Amino Methyl Propanol',
      id: 'amino_methyl_propanol',
      categories: ['emollient_alcohols'],
    },
    'amino-2-methyl-1-propanol': {
      name: 'Amino-2-Methyl-1-Propanol',
      id: 'amino-2-methyl-1-propanol',
      categories: ['emollient_alcohols'],
    },
    amylcinnamyl_alcohol: {
      name: 'Amylcinnamyl Alcohol',
      id: 'amylcinnamyl_alcohol',
      categories: ['emollient_alcohols'],
      synonyms: ['amyl cinnamyl alcohol'],
      references: ['https://cosdna.com/eng/3a86ec9991.html'],
    },
    arachidyl_alcohol: {
      name: 'Arachidyl Alcohol',
      id: 'arachidyl_alcohol',
      categories: ['emollient_alcohols'],
    },
    behenyl_alcohol: {
      name: 'Behenyl Alcohol',
      id: 'behenyl_alcohol',
      synonyms: ['docosanol', 'behenic alcohol'],
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/5fc5481.html'],
    },
    benzyl_alcohol: {
      name: 'Benzyl Alcohol',
      id: 'benzyl_alcohol',
      categories: ['emollient_alcohols'],
      references: ['https://incidecoder.com/ingredients/benzyl-alcohol'],
    },
    brassica_alcohol: {
      name: 'Brassica Alcohol',
      id: 'brassica_alcohol',
      categories: ['emollient_alcohols'],
    },
    butylene_alcohol: {
      name: 'Butylene Alcohol',
      id: 'butylene_alcohol',
      categories: ['emollient_alcohols'],
    },
    'c14-22_alcohol': {
      name: 'C14-22 Alcohol',
      id: 'c14-22_alcohol',
      categories: ['emollient_alcohols'],
    },
    'c20-c22_alcohol': {
      name: 'C20-C22 Alcohol',
      id: 'c20-c22_alcohol',
      categories: ['emollient_alcohols'],
    },
    'c30-50_alcohols': {
      name: 'C30-50 Alcohols',
      id: 'c30-50_alcohols',
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/8fb0237001.html'],
    },
    cetearyl_alcohol: {
      name: 'Cetearyl Alcohol',
      id: 'cetearyl_alcohol',
      synonyms: [
        'cetyl stearyl alcohol',
        'cetostearyl alcohol',
        'c16-18 alcohols',
        'ceteryl alcohol',
      ],
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/f83898102.html'],
    },
    cetyl_alcohol: {
      name: 'Cetyl Alcohol',
      id: 'cetyl_alcohol',
      synonyms: [
        'cetanol',
        '1-hexadecanol',
        'palmityl alcohol',
        'hexadecyl alcohol',
        'palmitoryl alcohol',
        'c16 alcohol l',
      ],
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/cac655104.html'],
    },
    cetylated_lanolin_alcohol: {
      name: 'Cetylated Lanolin Alcohol',
      id: 'cetylated_lanolin_alcohol',
      categories: ['emollient_alcohols'],
    },
    cinnamyl_alcohol: {
      name: 'Cinnamyl Alcohol',
      id: 'cinnamyl_alcohol',
      synonyms: ['cinnamic alcohol'],
      categories: ['emollient_alcohols'],
      references: ['https://incidecoder.com/ingredients/cinnamyl-alcohol'],
    },
    coconut_alcohol: {
      name: 'Coconut Alcohol',
      id: 'coconut_alcohol',
      categories: ['emollient_alcohols'],
    },
    ethyl_alcohol: {
      name: 'Ethyl Alcohol',
      id: 'ethyl_alcohol',
      synonyms: [
        'grain alcohol',
        'sugarcane-derived alcohol',
        'sugarcane alcohol',
      ],
      categories: ['drying_alcohols'],
      references: ['https://cosdna.com/eng/554009929.html'],
    },
    isocetyl_alcohol: {
      name: 'Isocetyl Alcohol',
      id: 'isocetyl_alcohol',
      synonyms: ['isohexadecanol'],
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/ac44ce1816.html'],
    },
    isopropanol: {
      name: 'Isopropanol',
      id: 'isopropanol',
      synonyms: ['isopropyl alcohol', '2-propanol'],
      categories: ['drying_alcohols'],
      references: ['https://cosdna.com/eng/e45cff199.html'],
    },
    isostearyl_alcohol: {
      name: 'Isostearyl Alcohol',
      id: 'isostearyl_alcohol',
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/78de01202.html'],
    },
    lanolin_alcohol: {
      name: 'Lanolin Alcohol',
      id: 'lanolin_alcohol',
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/cd20031856.html'],
    },
    lauryl_alcohol: {
      name: 'Lauryl Alcohol',
      id: 'lauryl_alcohol',
      categories: ['emollient_alcohols'],
      synonyms: ['1-dodecanol', 'dodecanol', 'dodecyl alcohol'],
      references: ['https://cosdna.com/eng/3a364c2.html'],
    },
    myristyl_alcohol: {
      name: 'Myristyl Alcohol',
      id: 'myristyl_alcohol',
      synonyms: ['1-tetradecanol', 'tetradecan-1-ol'],
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/76640d1970.html'],
    },
    oleyl_alcohol: {
      name: 'Oleyl Alcohol',
      id: 'oleyl_alcohol',
      categories: ['emollient_alcohols'],
    },
    pantothenyl_alcohol: {
      name: 'Pantothenyl Alcohol',
      id: 'pantothenyl_alcohol',
      categories: ['emollient_alcohols'],
    },
    phenethyl_alcohol: {
      name: 'Phenethyl Alcohol',
      id: 'phenethyl_alcohol',
      synonyms: ['phenylethyl alcohol', 'phenyl ethyl alcohol'],
      categories: ['preservative_alcohols'],
    },
    phenyl_propanol: {
      name: 'Phenyl Propanol',
      id: 'phenyl_propanol',
      synonyms: ['phenylpropanol'],
      categories: ['emollient_alcohols'],
    },
    propyl_alcohol: {
      name: 'Propyl Alcohol',
      id: 'propyl_alcohol',
      categories: ['drying_alcohols'],
      references: ['https://cosdna.com/eng/366a832503.html'],
    },
    sd_alcohol: {
      name: 'SD Alcohol',
      id: 'sd_alcohol',
      synonyms: [
        'alcohol denat',
        'alcohol-40b',
        'denatured alcohol',
        'sd alcohol 40',
        'sugarcane derived alcohol',
        'alcohol',
      ],
      description:
        'Dries out your hair and skin, and can cause irritation and dryness.',
      categories: ['drying_alcohols'],
      references: ['https://cosdna.com/eng/554009929.html'],
    },
    steareth_alcohol: {
      name: 'Steareth Alcohol',
      id: 'steareth_alcohol',
      categories: ['emollient_alcohols'],
    },
    stearyl_alcohol: {
      name: 'Stearyl Alcohol',
      id: 'stearyl_alcohol',
      synonyms: ['1-octadecanol', 'octadecan-1-ol', 'steoryl alcohol'],
      categories: ['emollient_alcohols'],
      references: ['https://cosdna.com/eng/5ef6ef467.html'],
    },
    undecyl_alcohol: {
      name: 'Undecyl Alcohol',
      id: 'undecyl_alcohol',
      categories: ['emollient_alcohols'],
    },
    unknown_alcohol: {
      name: 'Unknown Alcohol',
      id: 'unknown_alcohol',
      synonyms: ['alcohol'],
      description:
        "Unknown alcohol, since we don't know what it is, we are going to classify it as drying alcohols just to be safe.",
      categories: ['drying_alcohols'],
    },
    alkylbenzene_sulfonate: {
      name: 'Alkylbenzene Sulfonate',
      id: 'alkylbenzene_sulfonate',
      synonyms: [
        'alkylbenzene sulphate',
        'alkyl benzene sulfonate',
        'alkyl benzene sulphate',
      ],
      categories: ['sulfonates'],
    },
    ammonium_cocoyl_isethionate: {
      name: 'Ammonium Cocoyl Isethionate',
      id: 'ammonium_cocoyl_isethionate',
      categories: ['mild_detergents'],
    },
    ammonium_cocoyl_sulfate: {
      name: 'Ammonium Cocoyl Sulfate',
      id: 'ammonium_cocoyl_sulfate',
      synonyms: ['ammonium cocoyl sulphate'],
      categories: ['sulfates'],
    },
    ammonium_laureth_sulfate: {
      name: 'Ammonium Laureth Sulfate',
      id: 'ammonium_laureth_sulfate',
      synonyms: ['ammonium laureth sulphate'],
      categories: ['sulfates'],
    },
    ammonium_lauryl_sulfate: {
      name: 'Ammonium Lauryl Sulfate',
      id: 'ammonium_lauryl_sulfate',
      synonyms: ['ammonium lauryl sulphate'],
      categories: ['sulfates'],
    },
    ammonium_xylenesulfonate: {
      name: 'Ammonium Xylenesulfonate',
      id: 'ammonium_xylenesulfonate',
      synonyms: [
        'ammonium xylenesulphate',
        'ammonium xylene-sulfonate',
        'ammonium xylene sulfonate',
      ],
      categories: ['sulfonates'],
    },
    babassuamidopropyl_betaine: {
      name: 'Babassuamidopropyl Betaine',
      id: 'babassuamidopropyl_betaine',
      categories: ['mild_detergents'],
    },
    capryl_glucoside: {
      name: 'Capryl Glucoside',
      id: 'capryl_glucoside',
      synonyms: ['capryl glucoside, caprylyl glucoside'],
      categories: ['mild_detergents'],
    },
    cocamidopropyl_betaine: {
      name: 'Cocamidopropyl Betaine',
      id: 'cocamidopropyl_betaine',
      categories: ['mild_detergents'],
    },
    cocamidopropyl_hydroxysultaine: {
      name: 'Cocamidopropyl Hydroxysultaine',
      id: 'cocamidopropyl_hydroxysultaine',
      categories: ['mild_detergents'],
    },
    coco_betaine: {
      name: 'Coco Betaine',
      id: 'coco_betaine',
      categories: ['mild_detergents'],
    },
    coco_glucoside: {
      name: 'Coco Glucoside',
      id: 'coco_glucoside',
      categories: ['mild_detergents'],
    },
    decyl_glucoside: {
      name: 'Decyl Glucoside',
      id: 'decyl_glucoside',
      categories: ['mild_detergents'],
    },
    decyl_polyglucose: {
      name: 'Decyl Polyglucose',
      id: 'decyl_polyglucose',
      categories: ['mild_detergents'],
    },
    dioctyl_sodium_sulfosuccinate: {
      name: 'Dioctyl Sodium Sulfosuccinate',
      id: 'dioctyl_sodium_sulfosuccinate',
      categories: ['other_detergents'],
    },
    disodium_cocoamphodiacetate: {
      name: 'Disodium Cocoamphodiacetate',
      id: 'disodium_cocoamphodiacetate',
      categories: ['mild_detergents'],
    },
    disodium_cocoamphodipropionate: {
      name: 'Disodium Cocoamphodipropionate',
      id: 'disodium_cocoamphodipropionate',
      categories: ['mild_detergents'],
    },
    disodium_cocoyl_glutamate: {
      name: 'Disodium Cocoyl Glutamate',
      id: 'disodium_cocoyl_glutamate',
      categories: ['other_detergents'],
    },
    disodium_laureth_succinate: {
      name: 'Disodium Laureth Succinate',
      id: 'disodium_laureth_succinate',
      categories: ['mild_detergents'],
    },
    disodium_laureth_sulfosuccinate: {
      name: 'Disodium Laureth Sulfosuccinate',
      id: 'disodium_laureth_sulfosuccinate',
      categories: ['mild_detergents'],
    },
    disodium_lauryl_sulfosuccinate: {
      name: 'Disodium Lauryl Sulfosuccinate',
      id: 'disodium_lauryl_sulfosuccinate',
      categories: ['mild_detergents'],
    },
    'ethyl_peg-15_cocamine_sulfate': {
      name: 'Ethyl PEG-15 Cocamine Sulfate',
      id: 'ethyl_peg-15_cocamine_sulfate',
      synonyms: ['ethyl peg-15 cocamine sulphate'],
      categories: ['sulfates'],
    },
    lauryl_glucoside: {
      name: 'Lauryl Glucoside',
      id: 'lauryl_glucoside',
      categories: ['mild_detergents'],
    },
    lauryl_hydroxysultaine: {
      name: 'Lauryl Hydroxysultaine',
      id: 'lauryl_hydroxysultaine',
      categories: ['mild_detergents'],
    },
    sodium_alkyl_sulfate: {
      name: 'Sodium Alkyl Sulfate',
      id: 'sodium_alkyl_sulfate',
      synonyms: ['sodium alkyl sulphate'],
      categories: ['sulfates'],
    },
    'sodium_c12-14_olefin_sulfonate': {
      name: 'Sodium C12-14 Olefin Sulfonate',
      id: 'sodium_c12-14_olefin_sulfonate',
      synonyms: ['sodium c12-14 olefin sulphate'],
      categories: ['sulfonates'],
    },
    'sodium_c12-18_alkyl_sulfate': {
      name: 'Sodium C12-18 Alkyl Sulfate',
      id: 'sodium_c12-18_alkyl_sulfate',
      synonyms: ['sodium c12-18 alkyl sulphate'],
      categories: ['sulfates'],
    },
    'sodium_c14-15_olefin_sulfonate': {
      name: 'Sodium C14-15 Olefin Sulfonate',
      id: 'sodium_c14-15_olefin_sulfonate',
      synonyms: ['sodium c14-15 olefin sulphate'],
      categories: ['sulfonates'],
    },
    'sodium_c14-16_olefin_sulfonate': {
      name: 'Sodium C14-16 Olefin Sulfonate',
      id: 'sodium_c14-16_olefin_sulfonate',
      synonyms: ['sodium c14-16 olefin sulphate'],
      categories: ['sulfonates'],
    },
    'sodium_c14-18_olefin_sulfonate': {
      name: 'Sodium C14-18 Olefin Sulfonate',
      id: 'sodium_c14-18_olefin_sulfonate',
      synonyms: ['sodium c14-18 olefin sulphate'],
      categories: ['sulfonates'],
    },
    'sodium_c14-26_olefin_sulfonate': {
      name: 'Sodium C14-26 Olefin Sulfonate',
      id: 'sodium_c14-26_olefin_sulfonate',
      synonyms: ['sodium c14-26 olefin sulphate'],
      categories: ['sulfonates'],
    },
    'sodium_c16-18_olefin_sulfonate': {
      name: 'Sodium C16-18 Olefin Sulfonate',
      id: 'sodium_c16-18_olefin_sulfonate',
      synonyms: ['sodium c16-18 olefin sulphate'],
      categories: ['sulfonates'],
    },
    sodium_cetearyl_sulfate: {
      name: 'Sodium Cetearyl Sulfate',
      id: 'sodium_cetearyl_sulfate',
      synonyms: ['sodium cetearyl sulphate'],
      categories: ['sulfates'],
    },
    sodium_coceth_sulfate: {
      name: 'Sodium Coceth Sulfate',
      id: 'sodium_coceth_sulfate',
      synonyms: ['sodium coceth sulphate'],
      categories: ['sulfates'],
    },
    sodium_coco_sulfate: {
      name: 'Sodium Coco Sulfate',
      id: 'sodium_coco_sulfate',
      synonyms: ['sodium coco sulphate'],
      categories: ['sulfates'],
    },
    sodium_cocoamphoacetate: {
      name: 'Sodium Cocoamphoacetate',
      id: 'sodium_cocoamphoacetate',
      categories: ['mild_detergents'],
    },
    sodium_cocosulfate: {
      name: 'Sodium Cocosulfate',
      id: 'sodium_cocosulfate',
      synonyms: ['sodium cocosulphate'],
      categories: ['sulfates'],
    },
    sodium_cocoyl_glutamate: {
      name: 'Sodium Cocoyl Glutamate',
      id: 'sodium_cocoyl_glutamate',
      categories: ['other_detergents'],
    },
    sodium_cocoyl_isethionate: {
      name: 'Sodium Cocoyl Isethionate',
      id: 'sodium_cocoyl_isethionate',
      categories: ['mild_detergents'],
    },
    sodium_cocoyl_sarcosinate: {
      name: 'Sodium Cocoyl Sarcosinate',
      id: 'sodium_cocoyl_sarcosinate',
      categories: ['other_detergents'],
    },
    sodium_laureth_sulfate: {
      name: 'Sodium Laureth Sulfate',
      id: 'sodium_laureth_sulfate',
      synonyms: ['sodium laureth sulphate'],
      categories: ['sulfates'],
    },
    'sodium_laureth-40_sulfate': {
      name: 'Sodium Laureth-40 Sulfate',
      id: 'sodium_laureth-40_sulfate',
      synonyms: ['sodium laureth-40 sulphate'],
      categories: ['sulfonates'],
    },
    sodium_lauroamphoacetate: {
      name: 'Sodium Lauroamphoacetate',
      id: 'sodium_lauroamphoacetate',
      categories: ['mild_detergents'],
    },
    sodium_lauroyl_glutamate: {
      name: 'Sodium Lauroyl Glutamate',
      id: 'sodium_lauroyl_glutamate',
      categories: ['mild_detergents'],
    },
    sodium_lauroyl_methyl_isethionate: {
      name: 'Sodium Lauroyl Methyl Isethionate',
      id: 'sodium_lauroyl_methyl_isethionate',
      synonyms: [
        'sodium lauryl methyl isothionate',
        'sodium lauroyl methyl lsethionate',
        'sodium lauroylmethyl isethionate',
      ],
      categories: ['other_detergents'],
    },
    sodium_lauroyl_sarcosinate: {
      name: 'Sodium Lauroyl Sarcosinate',
      id: 'sodium_lauroyl_sarcosinate',
      categories: ['other_detergents'],
    },
    sodium_lauryl_glucose_carboxylate: {
      name: 'Sodium Lauryl Glucose Carboxylate',
      id: 'sodium_lauryl_glucose_carboxylate',
      categories: ['mild_detergents'],
    },
    sodium_lauryl_sarcosinate: {
      name: 'Sodium Lauryl Sarcosinate',
      id: 'sodium_lauryl_sarcosinate',
      synonyms: ['sodium lauroyl sarcosinate', 'sodium lauroyl sarcosine'],
      categories: ['other_detergents'],
    },
    sodium_lauryl_sulfate: {
      name: 'Sodium Lauryl Sulfate',
      id: 'sodium_lauryl_sulfate',
      synonyms: ['sodium lauryl sulphate'],
      categories: ['sulfates'],
    },
    sodium_lauryl_sulfoacetate: {
      name: 'Sodium Lauryl Sulfoacetate',
      id: 'sodium_lauryl_sulfoacetate',
      categories: ['other_detergents'],
    },
    sodium_laurylglucosides_hydroxypropylsulfonate: {
      name: 'Sodium Laurylglucosides Hydroxypropylsulfonate',
      id: 'sodium_laurylglucosides_hydroxypropylsulfonate',
      categories: ['mild_detergents'],
    },
    sodium_methyl_cocoyl_taurate: {
      name: 'Sodium Methyl Cocoyl Taurate',
      id: 'sodium_methyl_cocoyl_taurate',
      categories: ['mild_detergents'],
    },
    sodium_myreth_sulfate: {
      name: 'Sodium Myreth Sulfate',
      id: 'sodium_myreth_sulfate',
      synonyms: ['sodium myreth sulphate'],
      categories: ['sulfates'],
    },
    sodium_myristoyl_sarcosinate: {
      name: 'Sodium Myristoyl Sarcosinate',
      id: 'sodium_myristoyl_sarcosinate',
      categories: ['other_detergents'],
    },
    sodium_polystyrene_sulfate: {
      name: 'Sodium Polystyrene Sulfate',
      id: 'sodium_polystyrene_sulfate',
      synonyms: ['sodium polystyrene sulphate'],
      categories: ['sulfates'],
    },
    sodium_xylenesulfonate: {
      name: 'Sodium Xylenesulfonate',
      id: 'sodium_xylenesulfonate',
      synonyms: [
        'sodium xylenesulphate',
        'sodium xylene-sulfonate',
        'sodium xylene sulfonate',
      ],
      categories: ['sulfonates'],
    },
    tea_dodecylbenzenesulfonate: {
      name: 'Tea Dodecylbenzenesulfonate',
      id: 'tea_dodecylbenzenesulfonate',
      synonyms: ['tea-dodecylbenzenesulfonate'],
      categories: ['sulfonates'],
    },
    tea_lauryl_sulfate: {
      name: 'Tea Lauryl Sulfate',
      id: 'tea_lauryl_sulfate',
      synonyms: ['tea lauryl sulphate'],
      categories: ['sulfates'],
    },
    triethanolamine_lauryl_sulfate: {
      name: 'Triethanolamine Lauryl Sulfate',
      id: 'triethanolamine_lauryl_sulfate',
      synonyms: ['triethanolamine lauryl sulphate'],
      categories: ['sulfates'],
    },
    apricot_kernel_oil: {
      name: 'Apricot Kernel Oil',
      id: 'apricot_kernel_oil',
      synonyms: ['prunus armeniaca kernel oil', 'apricot seed oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/prunus-armeniaca-kernel-oil',
      ],
    },
    argan_oil: {
      name: 'Argan Oil',
      id: 'argan_oil',
      synonyms: ['argania spinosa kernel oil', 'argan tree oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/argania-spinosa-kernel-oil',
      ],
    },
    avocado_oil: {
      name: 'Avocado Oil',
      id: 'avocado_oil',
      synonyms: ['persea gratissima oil', 'avocado fruit oil'],
      categories: ['medium_oils'],
      references: ['https://incidecoder.com/ingredients/persea-gratissima-oil'],
    },
    baobab_oil: {
      name: 'Baobab Oil',
      id: 'baobab_oil',
      synonyms: ['adansonia digitata seed oil', 'baobab seed oil'],
      categories: ['medium_oils'],
      references: [
        'https://incidecoder.com/ingredients/adansonia-digitata-seed-oil',
      ],
    },
    camellia_oil: {
      name: 'Camellia Oil',
      id: 'camellia_oil',
      synonyms: ['camellia oleifera seed oil', 'tea seed oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/camellia-oleifera-seed-oil',
      ],
    },
    castor_oil: {
      name: 'Castor Oil',
      id: 'castor_oil',
      synonyms: ['ricinus communis seed oil', 'castor seed oil'],
      categories: ['heavy_oils'],
      references: [
        'https://incidecoder.com/ingredients/ricinus-communis-seed-oil',
      ],
    },
    coconut_oil: {
      name: 'Coconut Oil',
      id: 'coconut_oil',
      synonyms: ['cocos nucifera oil', 'coconut kernel oil'],
      categories: ['medium_oils'],
      references: ['https://incidecoder.com/ingredients/cocos-nucifera-oil'],
    },
    grapeseed_oil: {
      name: 'Grapeseed Oil',
      id: 'grapeseed_oil',
      synonyms: ['vitis vinifera seed oil', 'grape seed oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/vitis-vinifera-seed-oil',
      ],
    },
    isohexadecane: {
      name: 'Isohexadecane',
      id: 'isohexadecane',
      synonyms: [
        'c16 hydrocarbon',
        'volatile hydrocarbon',
        'isohexanedecane',
        'isohexad',
      ],
      categories: ['light_oils'],
      references: ['https://incidecoder.com/ingredients/isohexadecane'],
    },
    jojoba_oil: {
      name: 'Jojoba Oil',
      id: 'jojoba_oil',
      synonyms: ['simmondsia chinensis seed oil', 'jojoba seed oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/simmondsia-chinensis-seed-oil',
      ],
    },
    lanolin: {
      name: 'Lanolin',
      id: 'lanolin',
      synonyms: ['wool wax', 'wool grease'],
      categories: ['heavy_oils'],
      references: ['https://incidecoder.com/ingredients/lanolin'],
    },
    marula_oil: {
      name: 'Marula Oil',
      id: 'marula_oil',
      synonyms: ['sclerocarya birrea seed oil', 'marula seed oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/sclerocarya-birrea-seed-oil',
      ],
    },
    mineral_oil: {
      name: 'Mineral Oil',
      id: 'mineral_oil',
      synonyms: ['paraffinum liquidum', 'liquid paraffin', 'petroleum oil'],
      categories: ['heavy_oils'],
      references: ['https://incidecoder.com/ingredients/mineral-oil'],
    },
    olive_oil: {
      name: 'Olive Oil',
      id: 'olive_oil',
      synonyms: ['olea europaea fruit oil', 'olive fruit oil'],
      categories: ['medium_oils'],
      references: [
        'https://incidecoder.com/ingredients/olea-europaea-fruit-oil',
      ],
    },
    paraffin: {
      name: 'Paraffin',
      id: 'paraffin',
      synonyms: ['paraffin wax', 'petroleum wax'],
      categories: ['heavy_oils'],
      references: ['https://incidecoder.com/ingredients/paraffin'],
    },
    petrolatum: {
      name: 'Petrolatum',
      id: 'petrolatum',
      synonyms: ['petroleum jelly', 'vaseline'],
      categories: ['heavy_oils'],
      references: ['https://incidecoder.com/ingredients/petrolatum'],
    },
    shea_butter_oil: {
      name: 'Shea Butter Oil',
      id: 'shea_butter_oil',
      synonyms: ['butyrospermum parkii butter oil', 'shea oil'],
      categories: ['medium_oils'],
      references: [
        'https://incidecoder.com/ingredients/butyrospermum-parkii-butter-oil',
      ],
    },
    sunflower_oil: {
      name: 'Sunflower Oil',
      id: 'sunflower_oil',
      synonyms: ['helianthus annuus seed oil', 'sunflower seed oil'],
      categories: ['medium_oils'],
      references: [
        'https://incidecoder.com/ingredients/helianthus-annuus-seed-oil',
      ],
    },
    sweet_almond_oil: {
      name: 'Sweet Almond Oil',
      id: 'sweet_almond_oil',
      synonyms: ['prunus amygdalus dulcis oil', 'almond oil'],
      categories: ['light_oils'],
      references: [
        'https://incidecoder.com/ingredients/prunus-amygdalus-dulcis-oil',
      ],
    },
    aminomethyl_propanol: {
      name: 'Aminomethyl Propanol',
      id: 'aminomethyl_propanol',
      categories: ['other_detergents'],
      references: ['https://incidecoder.com/ingredients/aminomethyl-propanol'],
    },
    'disodium_2-sulfolaurate': {
      name: 'Disodium 2-Sulfolaurate',
      id: 'disodium_2-sulfolaurate',
      categories: ['other_detergents'],
    },
    potassium_castorate: {
      name: 'Potassium Castorate',
      id: 'potassium_castorate',
      synonyms: ['potassium salt of castor oil fatty acids', 'castor oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/potassium-castorate'],
    },
    potassium_cocoate: {
      name: 'Potassium Cocoate',
      id: 'potassium_cocoate',
      synonyms: [
        'potassium salt of coconut oil fatty acids',
        'coconut oil soap',
      ],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/potassium-cocoate'],
    },
    potassium_olivate: {
      name: 'Potassium Olivate',
      id: 'potassium_olivate',
      synonyms: ['potassium salt of olive oil fatty acids', 'olive oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/potassium-olivate'],
    },
    potassium_palmate: {
      name: 'Potassium Palmate',
      id: 'potassium_palmate',
      synonyms: ['potassium salt of palmitic acid', 'palm oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/potassium-palmate'],
    },
    potassium_stearate: {
      name: 'Potassium Stearate',
      id: 'potassium_stearate',
      synonyms: ['potassium salt of stearic acid', 'potassium soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/potassium-stearate'],
    },
    saccharomycessilicon_ferment: {
      name: 'Saccharomycessilicon Ferment',
      id: 'saccharomycessilicon_ferment',
      categories: ['other_detergents'],
    },
    silicon_ferment: {
      name: 'Silicon Ferment',
      id: 'silicon_ferment',
      categories: ['other_detergents'],
    },
    soaps: {
      name: 'Soap',
      id: 'soaps',
      categories: ['other_detergents'],
    },
    sodium_castorate: {
      name: 'Sodium Castorate',
      id: 'sodium_castorate',
      synonyms: ['sodium salt of castor oil fatty acids', 'castor oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/sodium-castorate'],
    },
    sodium_cocoate: {
      name: 'Sodium Cocoate',
      id: 'sodium_cocoate',
      synonyms: ['sodium salt of coconut oil fatty acids', 'coconut oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/sodium-cocoate'],
    },
    sodium_lauroyl_hydrolyzed_silk: {
      name: 'Sodium Lauroyl Hydrolyzed Silk',
      id: 'sodium_lauroyl_hydrolyzed_silk',
      categories: ['other_detergents'],
    },
    sodium_lauroyl_lactylate: {
      name: 'Sodium Lauroyl Lactylate',
      id: 'sodium_lauroyl_lactylate',
      categories: ['other_detergents'],
    },
    sodium_lauroyl_oat_amino_acids: {
      name: 'Sodium Lauroyl Oat Amino Acids',
      id: 'sodium_lauroyl_oat_amino_acids',
      categories: ['other_detergents'],
    },
    'sodium_methyl_2-sulfolaurate': {
      name: 'Sodium Methyl 2-Sulfolaurate',
      id: 'sodium_methyl_2-sulfolaurate',
      categories: ['other_detergents'],
    },
    sodium_olivate: {
      name: 'Sodium Olivate',
      id: 'sodium_olivate',
      synonyms: ['sodium salt of olive oil fatty acids', 'olive oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/sodium-olivate'],
    },
    sodium_palmate: {
      name: 'Sodium Palmate',
      id: 'sodium_palmate',
      synonyms: ['sodium salt of palmitic acid', 'palm oil soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/sodium-palmate'],
    },
    sodium_stearate: {
      name: 'Sodium Stearate',
      id: 'sodium_stearate',
      synonyms: ['sodium salt of stearic acid', 'stearic acid soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/sodium-stearate'],
    },
    sodium_tallowate: {
      name: 'Sodium Tallowate',
      id: 'sodium_tallowate',
      synonyms: ['sodium salt of tallow fatty acids', 'tallow soap'],
      categories: ['soaps'],
      references: ['https://incidecoder.com/ingredients/sodium-tallowate'],
    },
    triisopropanolamine: {
      name: 'Triisopropanolamine',
      id: 'triisopropanolamine',
      categories: ['other_detergents'],
    },
    witch_hazel: {
      name: 'Witch Hazel',
      id: 'witch_hazel',
      synonyms: ['witch', 'hamamelis', 'hamamellis', 'hazel'],
      categories: ['astringents'],
    },
    unknown_paraben: {
      name: 'Unknown Paraben',
      id: 'unknown_paraben',
      synonyms: ['paraben'],
      categories: ['parabens'],
    },
    amodimethicone: {
      name: 'Amodimethicone',
      id: 'amodimethicone',
      synonyms: ['amine-functional dimethicone', 'amino-modified silicone'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/amodimethicone'],
    },
    bis_peg_18_methyl_ether_dimethyl_silane: {
      name: 'Bis-PEG-18 Methyl Ether Dimethyl Silane',
      id: 'bis_peg_18_methyl_ether_dimethyl_silane',
      synonyms: [
        'polyethylene glycol-18 methyl ether dimethyl silane',
        'dimethylsilane glycol ether',
      ],
      categories: ['water-soluble_silicones'],
      references: [
        'https://incidecoder.com/ingredients/bis-peg-18-methyl-ether-dimethyl-silane',
      ],
    },
    cetearyl_methicone: {
      name: 'Cetearyl Methicone',
      id: 'cetearyl_methicone',
      synonyms: ['cetearyl dimethicone', 'methicone ester'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/cetearyl-methicone'],
    },
    cyclohexasiloxane: {
      name: 'Cyclohexasiloxane',
      id: 'cyclohexasiloxane',
      synonyms: ['d6', 'dodecamethylcyclohexasiloxane', 'cyclohexamethicone'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/cyclohexasiloxane'],
    },
    cyclopentasiloxane: {
      name: 'Cyclopentasiloxane',
      id: 'cyclopentasiloxane',
      synonyms: ['d5', 'decamethylcyclopentasiloxane', 'cyclopentamethicone'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/cyclopentasiloxane'],
    },
    dimethicone: {
      name: 'Dimethicone',
      id: 'dimethicone',
      synonyms: ['polydimethylsiloxane', 'pdms', 'dimethylpolysiloxane'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/dimethicone'],
    },
    dimethicone_copolyol: {
      name: 'Dimethicone Copolyol',
      id: 'dimethicone_copolyol',
      synonyms: [
        'dimethicone polyethylene glycol ether',
        'dimethicone polyoxyethylene',
      ],
      categories: ['water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/dimethicone-copolyol'],
    },
    dimethiconol: {
      name: 'Dimethiconol',
      id: 'dimethiconol',
      synonyms: ['hydroxyl-terminated dimethicone', 'dimethylsiloxane polymer'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/dimethiconol'],
    },
    lauryl_methicone_copolyol: {
      name: 'Lauryl Methicone Copolyol',
      id: 'lauryl_methicone_copolyol',
      synonyms: [
        'lauryl polyethylene glycol dimethicone',
        'polyoxyethylene lauryl dimethicone',
      ],
      categories: ['water-soluble_silicones'],
      references: [
        'https://incidecoder.com/ingredients/lauryl-methicone-copolyol',
      ],
    },
    peg_10_dimethicone: {
      name: 'PEG-10 Dimethicone',
      id: 'peg_10_dimethicone',
      synonyms: ['polyethylene glycol-10 dimethicone', 'dimethicone polyether'],
      categories: ['water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/peg-10-dimethicone'],
    },
    peg_12_dimethicone: {
      name: 'PEG-12 Dimethicone',
      id: 'peg_12_dimethicone',
      synonyms: [
        'polyethylene glycol-12 dimethicone',
        'dimethicone ethoxylate',
      ],
      categories: ['water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/peg-12-dimethicone'],
    },
    peg_14_dimethicone: {
      name: 'PEG-14 Dimethicone',
      id: 'peg_14_dimethicone',
      synonyms: [
        'polyethylene glycol-14 dimethicone',
        'dimethicone glycol ether',
      ],
      categories: ['water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/peg-14-dimethicone'],
    },
    peg_7_amodimethicone: {
      name: 'PEG-7 Amodimethicone',
      id: 'peg_7_amodimethicone',
      synonyms: [
        'polyether modified amodimethicone',
        'polyethylene glycol-7 amodimethicone',
      ],
      categories: ['water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/peg-7-amodimethicone'],
    },
    peg_8_dimethicone: {
      name: 'PEG-8 Dimethicone',
      id: 'peg_8_dimethicone',
      synonyms: [
        'polyethylene glycol-8 dimethicone',
        'peg-modified dimethicone',
      ],
      categories: ['water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/peg-8-dimethicone'],
    },
    phenyl_trimethicone: {
      name: 'Phenyl Trimethicone',
      id: 'phenyl_trimethicone',
      synonyms: ['trimethylsilylphenyl dimethicone', 'phenylmethylsilicone'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/phenyl-trimethicone'],
    },
    silicone_quaternium_8: {
      name: 'Silicone Quaternium-8',
      id: 'silicone_quaternium_8',
      synonyms: ['cationic silicone polymer', 'quaternized dimethicone'],
      categories: ['non-water-soluble_silicones'],
      references: ['https://incidecoder.com/ingredients/silicone-quaternium-8'],
    },
    stearoxytrimethylsilane: {
      name: 'Stearoxytrimethylsilane',
      id: 'stearoxytrimethylsilane',
      synonyms: ['octadecyltrimethylsilane', 'stearic acid trimethylsilane'],
      categories: ['non-water-soluble_silicones'],
      references: [
        'https://incidecoder.com/ingredients/stearoxytrimethylsilane',
      ],
    },
    trimethylsiloxyamodimethicone: {
      name: 'Trimethylsiloxyamodimethicone',
      id: 'trimethylsiloxyamodimethicone',
      synonyms: ['amino silicone resin', 'amino-functionalized silicone'],
      categories: ['non-water-soluble_silicones'],
      references: [
        'https://incidecoder.com/ingredients/trimethylsiloxyamodimethicone',
      ],
    },
    trimethylsiloxysilicate: {
      name: 'Trimethylsiloxysilicate',
      id: 'trimethylsiloxysilicate',
      categories: ['non-water-soluble_silicones'],
    },
    cyclomethicone: {
      name: 'Cyclomethicone',
      id: 'cyclomethicone',
      synonyms: ['cyclicdimethylpolysiloxane', 'polydimethylcyclosiloxane'],
      categories: ['evaporative_silicones'],
    },
    unknown_peg_silicone: {
      name: 'Unknown PEG Silicone',
      id: 'unknown_peg_silicone',
      synonyms: ['PPG-', 'PEG-', 'PEG/PPG'],
      categories: ['water-soluble_silicones'],
    },
    unknown_non_water_soluble_silicones: {
      name: 'Unknown Non-Water Soluble Silicones',
      id: 'unknown_non_water_soluble_silicones',
      synonyms: [
        'cone',
        'dimethicon',
        'silane',
        'siloxane',
        'dimethcione',
        'botanisil',
        'silicon',
        'silylate',
        'silsesquioxane',
        'siloxysilicate',
        'microsil',
      ],
      categories: ['non-water-soluble_silicones'],
    },
    unknown_water_soluble_silicones: {
      name: 'Unknown Water Soluble Silicones',
      id: 'unknown_water_soluble_silicones',
      synonyms: ['peg', 'ppg', 'pg-'],
      categories: ['water-soluble_silicones'],
    },
    unknown_sulfates: {
      name: 'Unknown Sulfates',
      id: 'unknown_sulfates',
      synonyms: ['sulfate', 'sulphate'],
      categories: ['sulfates'],
    },
    unknown_sarcosinate: {
      name: 'Unknown Sarcosinate',
      id: 'unknown_sarcosinate',
      synonyms: ['sarcosinate'],
      categories: ['other_detergents'],
    },
    unknown_cocoyl_glutamate: {
      name: 'Unknown Cocoyl Glutamate',
      id: 'unknown_cocoyl_glutamate',
      synonyms: ['cocoyl glutamate'],
      categories: ['other_detergents'],
    },
    unknown_wax: {
      name: 'Unknown Wax',
      id: 'unknown_wax',
      synonyms: ['wax'],
      categories: ['non-water-soluble_waxes'],
    },
    beeswax: {
      name: 'Beeswax',
      id: 'beeswax',
      synonyms: ['cera alba', 'natural wax from bees', 'cire dabeille'],
      categories: ['non-water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/beeswax'],
    },
    candelilla_wax: {
      name: 'Candelilla Wax',
      id: 'candelilla_wax',
      synonyms: [
        'euphorbia cerifera wax',
        'vegetable wax from candelilla shrub',
      ],
      categories: ['non-water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/candelilla-wax'],
    },
    carnauba_wax: {
      name: 'Carnauba Wax',
      id: 'carnauba_wax',
      synonyms: ['copernicia cerifera wax', 'palm wax'],
      categories: ['non-water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/carnauba-wax'],
    },
    ceteareth_20: {
      name: 'Ceteareth-20',
      id: 'ceteareth_20',
      synonyms: [
        'polyoxyethylene cetearyl ether',
        'ethoxylated cetearyl alcohol',
      ],
      categories: ['water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/ceteareth-20'],
    },
    euphorbia_cerifera_wax: {
      name: 'Euphorbia Cerifera Wax',
      id: 'euphorbia_cerifera_wax',
      synonyms: ['candelilla wax', 'vegetable wax', 'natural plant wax'],
      categories: ['non-water-soluble_waxes'],
      references: [
        'https://incidecoder.com/ingredients/euphorbia-cerifera-wax',
      ],
    },
    microcrystalline_wax: {
      name: 'Microcrystalline Wax',
      id: 'microcrystalline_wax',
      synonyms: ['petroleum wax', 'mineral wax'],
      categories: ['non-water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/microcrystalline-wax'],
    },
    ozokerite: {
      name: 'Ozokerite',
      id: 'ozokerite',
      synonyms: ['mineral wax', 'earth wax'],
      categories: ['non-water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/ozokerite'],
    },
    paraffin_wax: {
      name: 'Paraffin Wax',
      id: 'paraffin_wax',
      synonyms: ['paraffin', 'petroleum-derived wax'],
      categories: ['non-water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/paraffin'],
    },
    peg_12_carnauba_wax: {
      name: 'PEG-12 Carnauba Wax',
      id: 'peg_12_carnauba_wax',
      synonyms: [
        'polyethylene glycol-12 carnauba wax',
        'ethoxylated carnauba wax',
      ],
      categories: ['water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/peg-12-carnauba-wax'],
    },
    peg_40_hydrogenated_castor_oil: {
      name: 'PEG-40 Hydrogenated Castor Oil',
      id: 'peg_40_hydrogenated_castor_oil',
      synonyms: [
        'polyethylene glycol-40 hydrogenated castor oil',
        'ethoxylated castor oil',
      ],
      categories: ['water-soluble_waxes'],
      references: [
        'https://incidecoder.com/ingredients/peg-40-hydrogenated-castor-oil',
      ],
    },
    peg_8_beeswax: {
      name: 'PEG-8 Beeswax',
      id: 'peg_8_beeswax',
      synonyms: ['polyethylene glycol-8 beeswax', 'ethoxylated beeswax'],
      categories: ['water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/peg-8-beeswax'],
    },
    polysorbate_20: {
      name: 'Polysorbate 20',
      id: 'polysorbate_20',
      synonyms: [
        'sorbitan monolaurate ethoxylated',
        'ethoxylated sorbitol laurate',
      ],
      categories: ['water-soluble_waxes'],
      references: ['https://incidecoder.com/ingredients/polysorbate-20'],
    },
  },
  categories: {
    drying_alcohols: {
      id: 'drying_alcohols',
      name: 'Drying Alcohols',
      group: 'alcohols',
      description:
        'Drying alcohols are used to remove moisture from the hair and skin, and can cause irritation and dryness.',
    },
    parabens: {
      id: 'parabens',
      name: 'Parabens',
      group: 'preservatives',
      description:
        'Parabens are a type of preservative that can cause irritation and dryness.',
    },
    sulfates: {
      id: 'sulfates',
      name: 'Sulfates',
      group: 'detergents',
      description:
        'Sulfates are a type of detergent that can cause irritation and dryness.',
    },
    sulfonates: {
      id: 'sulfonates',
      name: 'Sulfonates',
      group: 'detergents',
      description:
        'Sulfonates are a type of detergent that can cause irritation and dryness.',
    },
    emollient_alcohols: {
      id: 'emollient_alcohols',
      name: 'Emollient alcohols',
      group: 'alcohols',
      description: 'Emollient alcohols are good for the skin and hair.',
    },
    'water-soluble_silicones': {
      id: 'water-soluble_silicones',
      name: 'Water-soluble Silicones',
      group: 'silicones',
      description: 'TBD',
    },
    'non-water-soluble_silicones': {
      id: 'non-water-soluble_silicones',
      name: 'Non-water-soluble Silicones',
      group: 'silicones',
      description: 'TBD',
    },
    preservatives: {
      id: 'preservatives',
      name: 'Preservatives',
      group: 'others',
      description: 'TBD',
    },
    other_detergents: {
      id: 'other_detergents',
      name: 'Other Detergents',
      group: 'detergents',
      description: 'TBD',
    },
    preservative_alcohols: {
      id: 'preservative_alcohols',
      name: 'Preservative Alcohols',
      group: 'alcohols',
      description: 'TBD',
    },
    astringents: {
      id: 'astringents',
      name: 'Astringents',
      group: 'others',
      description: 'TBD',
    },
    soaps: {
      id: 'soaps',
      name: 'Soaps',
      group: 'others',
      description: 'TBD',
    },
    mild_detergents: {
      id: 'mild_detergents',
      name: 'Mild Detergents',
      group: 'detergents',
      description: 'TBD',
    },
    heavy_oils: {
      id: 'heavy_oils',
      name: 'Heavy Oils',
      group: 'oils',
      description: 'TBD',
    },
    light_oils: {
      id: 'light_oils',
      name: 'Light Oils',
      group: 'oils',
      description: 'TBD',
    },
    medium_oils: {
      id: 'medium_oils',
      name: 'Medium Oils',
      group: 'oils',
      description: 'TBD',
    },
    'non-water-soluble_waxes': {
      id: 'non-water-soluble_waxes',
      name: 'Non-water-soluble Waxes',
      group: 'waxes',
      description: 'TBD',
    },
    'water-soluble_waxes': {
      id: 'water-soluble_waxes',
      name: 'Water-soluble Waxes',
      group: 'waxes',
      description: 'TBD',
    },
    evaporative_silicones: {
      id: 'evaporative_silicones',
      name: 'Evaporative Silicones',
      group: 'silicones',
      description: 'TBD',
    },
  },
  groups: {
    alcohols: {
      id: 'alcohols',
      name: 'Alcohols',
    },
    preservatives: {
      id: 'preservatives',
      name: 'Preservatives',
    },
    detergents: {
      id: 'detergents',
      name: 'Detergents',
    },
    silicones: {
      id: 'silicones',
      name: 'Silicones',
    },
    others: {
      id: 'others',
      name: 'Others',
    },
    waxes: {
      id: 'waxes',
      name: 'Waxes',
    },
    oils: {
      id: 'oils',
      name: 'Oils',
    },
  },
};

export const defaultSystems: System[] = [
  {
    name: 'Curly Default',
    id: 'curly_default',
    description:
      'The Curly Default system is a hair care system that focuses on using products that are free of harsh chemicals and sulfates. It recommends using products that are free of drying alcohols, parabens, and other potentially harmful ingredients.',
    settings: [
      'mild_detergents_only',
      'drying_alcohol',
      'silicone_free',
      'no_waxes',
      'no_heavy_oils',
      'paraben_free',
      'soap_free',
      'no_astringents',
    ],
  },
  {
    name: 'Curly Moderate',
    id: 'curly_moderate',
    description:
      'Just like the default system, but allows for some water soluble silicones and moderate surfactants.',
    settings: [
      'sulfate_free',
      'drying_alcohol',
      'no_water_insoluble_silicones',
      'no_waxes',
      'no_heavy_oils',
      'caution_water_soluble_silicones',
    ],
  },
  {
    name: 'No Poo',
    id: 'no_poo',
    description:
      'No Poo is a hair care system that focuses on using products that are free of harsh chemicals and sulfates. It recommends using products that are free of drying alcohols, parabens, and other potentially harmful ingredients.',
    ingredients: [],
    categories: [],
  },
  {
    name: 'Wavy Hair',
    id: 'wavy_hair',
    description:
      'Wavy hair needs a different approach to hair care. You may still need to use some sulfates and you should avoid heavy product and oils',
    ingredients: [],
    categories: ['oils'],
  },
];

export const defaultSettings: Record<string, Setting> = {
  sulfate_free: {
    id: 'sulfate_free',
    name: 'Sulfate Free',
    description:
      'Sulfates are a type of detergent that can cause irritation and dryness.',
    ingredients: [],
    categories: ['sulfates'],
    groups: [],
    flags: ['avoid'],
  },
  paraben_free: {
    id: 'paraben_free',
    name: 'Paraben Free',
    description:
      'Parabens are a type of preservative that can cause irritation and dryness.',
    ingredients: [],
    categories: ['parabens'],
    groups: [],
    flags: ['avoid'],
  },
  cleansing: {
    id: 'cleansing',
    name: 'Cleansing',
    description:
      'Cleansing ingredients help remove dirt and oil from the hair and scalp.',
    ingredients: [],
    categories: ['sulfates'],
    groups: [],
    flags: ['prefer'],
  },
  drying_alcohol: {
    id: 'drying_alcohol',
    name: 'No Drying Alcohol',
    description:
      'Drying alcohols are used to remove moisture from the hair and skin, and can cause irritation and dryness.',
    ingredients: [],
    categories: ['drying_alcohols'],
    groups: [],
    flags: ['avoid'],
  },
  silicone_free: {
    id: 'silicone_free',
    name: 'Silicone Free',
    description: 'Avoiding all silicones even water-soluble ones.',
    ingredients: [],
    categories: ['silicones'],
    groups: [],
    flags: ['avoid'],
  },
  no_heavy_oils: {
    id: 'no_heavy_oils',
    name: 'No Heavy Oils',
    description: 'Avoiding heavy oils that can weigh down the hair and scalp.',
    ingredients: [],
    categories: ['heavy_oils'],
    groups: [],
    flags: ['avoid'],
  },
  no_waxes: {
    id: 'no_waxes',
    name: 'No Waxes',
    description: 'Avoiding all non water soluble waxes.',
    ingredients: [],
    categories: ['non-water-soluble_waxes'],
    groups: [],
    flags: ['avoid'],
  },
  no_water_insoluble_silicones: {
    id: 'no_water_insoluble_silicones',
    name: 'No Water Insoluble Silicones',
    description: 'Avoiding all water insoluble silicones.',
    ingredients: [],
    categories: ['non-water-soluble_silicones'],
    groups: [],
    flags: ['avoid'],
  },
  no_strong_surfactants: {
    id: 'no_strong_surfactants',
    name: 'No Strong Surfactants',
    description: 'Avoiding all strong surfactants.',
    ingredients: [],
    categories: ['sulfonates', 'sulfates'],
    groups: [],
    flags: ['avoid'],
  },
  mild_detergents_only: {
    id: 'mild_detergents_only',
    name: 'Mild Detergents Only',
    description:
      'Only allows mild detergents, avoiding all other surfactants and cleansers.',
    ingredients: [],
    categories: ['mild_detergents'],
    groups: [],
    flags: ['avoid_others_in_category'],
  },
  caution_water_soluble_silicones: {
    id: 'caution_water_soluble_silicones',
    name: 'Caution Water Soluble Silicones',
    description: 'Flag the water soluble silicones as caution',
    ingredients: [],
    categories: ['water-soluble_silicones'],
    groups: [],
    flags: ['caution'],
  },
  soap_free: {
    id: 'soap_free',
    name: 'Soap Free',
    description: 'Avoiding all soaps.',
    ingredients: [],
    categories: ['soaps'],
    groups: [],
    flags: ['avoid'],
  },
  no_astringents: {
    id: 'no_astringents',
    name: 'No Astringents',
    description: 'Avoiding all astringents.',
    ingredients: [],
    categories: ['astringents'],
    groups: [],
    flags: ['avoid'],
  },
};

export function getBundledDatabase(): IngredientDatabase {
  return defaultDatabase;
}

export function getBundledSystems(): System[] {
  return defaultSystems;
}

export function getBundledSettings(): Record<string, Setting> {
  return defaultSettings;
}
